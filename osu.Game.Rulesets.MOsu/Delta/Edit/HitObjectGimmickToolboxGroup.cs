...
namespace osu.Game.Rulesets.MOsu.Delta.Edit
{
    public partial class HitObjectGimmickToolboxGroup : EditorToolboxGroup
    {
        ...
        private DeltaFormSliderBar<float> fakeRevealStrength = null!;
                ...
                Children = new Drawable[]
                {
                    ...
                    fakeNoteFields = createContainer(
                        ...
                        fakeRevealFields = createContainer(
                            fakeRevealStrength = new DeltaFormSliderBar<float>
                            {
        ...
        private void bindSlider(DeltaFormSliderBar<float> source, Action<osu.Game.Rulesets.MOsu.Delta.Gimmicks.HitObjectGimmickSettings, float> setter, Func<float, float> clamp)
            => source.Current.BindValueChanged(v => setSlider(source, setter, v.NewValue, clamp));

        private void bindSlider(DeltaFormSliderBar<double> source, Action<osu.Game.Rulesets.MOsu.Delta.Gimmicks.HitObjectGimmickSettings, double> setter, Func<double, double> clamp)
            => source.Current.BindValueChanged(v => setSlider(source, setter, v.NewValue, clamp));

        private void bindFloat(FormNumberBox source, Action<osu.Game.Rulesets.MOsu.Delta.Gimmicks.HitObjectGimmickSettings, float> setter, Func<float, float> clamp)
            => source.OnCommit += (_, _) => setFloat(source, setter, clamp);

        private void bindFloatOnCommitOnly(FormNumberBox source, Action<osu.Game.Rulesets.MOsu.Delta.Gimmicks.HitObjectGimmickSettings, float> setter, Func<float, float> clamp)
            => source.OnCommit += (_, _) => setFloat(source, setter, clamp);

        private void bindInt(FormNumberBox source, Action<osu.Game.Rulesets.MOsu.Delta.Gimmicks.HitObjectGimmickSettings, int> setter, Func<int, int> clamp)
        ...
        private void updateFromSelection()
        {
            ...
            try
            {
                ...
                var inList = selected.Select(h => (h, inBeatmap: editorBeatmap.HitObjects.Contains(h), id: DeltaGimmickApplier.GetObjectId(h))).ToList();
        ...
        private void setBool(bool value, Func<bool> current, Action<osu.Game.Rulesets.MOsu.Delta.Gimmicks.HitObjectGimmickSettings, bool> setter)
        {
        ...
        private void setFloat(FormNumberBox source, Action<osu.Game.Rulesets.MOsu.Delta.Gimmicks.HitObjectGimmickSettings, float> setter, Func<float, float> clamp)
        {
            if (updatingControls)
                return;
        ...
        private void setInt(FormNumberBox source, Action<osu.Game.Rulesets.MOsu.Delta.Gimmicks.HitObjectGimmickSettings, int> setter, Func<int, int> clamp)
        {
            if (updatingControls)
                return;
        ...
        private void setSlider(DeltaFormSliderBar<float> source,
                               Action<osu.Game.Rulesets.MOsu.Delta.Gimmicks.HitObjectGimmickSettings, float> setter,
                               float value,
        ...
        private void setSlider(DeltaFormSliderBar<double> source,
                               Action<osu.Game.Rulesets.MOsu.Delta.Gimmicks.HitObjectGimmickSettings, double> setter,
                               double value,
        ...
        private static void setEnabledState(bool enabled, params Drawable[] items)
        {
            foreach (var item in items)
            {
                switch (item)
                {
                    ...
                    case DeltaFormSliderBar<float> s:
                        s.Current.Disabled = !enabled;
                        break;

                    case DeltaFormSliderBar<double> s:
                        s.Current.Disabled = !enabled;
                        break;

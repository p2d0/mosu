...
{
    public partial class DeltaEditorChangeHandler : EditorChangeHandler
    {
        private readonly EditorBeatmap editorBeatmap;
        private readonly LegacyEditorBeatmapPatcher patcher;
        private readonly DeltaBeatmap? playable;

        public DeltaEditorChangeHandler(EditorBeatmap editorBeatmap)
        {
            this.editorBeatmap = editorBeatmap;
            playable = editorBeatmap.PlayableBeatmap as DeltaBeatmap;
        ...
        protected override void WriteCurrentStateToStream(MemoryStream stream)
        {
            ...
            using (var sw = new StreamWriter(stream, Encoding.UTF8, 1024, true))
            {
                ...
                if (playable != null)
                    sw.Write(DeltaGimmickSerializer.Serialize(playable.Gimmicks.Sections, playable.Gimmicks.HitObjectGimmicks));
            }
        }
        ...
        protected override void ApplyStateChange(byte[] previousState, byte[] newState)
        {
            ...
            (playable.Gimmicks.Sections, playable.Gimmicks.HitObjectGimmicks) = DeltaGimmickParser.Parse(reader);
            playable.Gimmicks.Applied = false;
            DeltaGimmickApplier.Apply(playable, playable.Gimmicks, mutateList: false);
        }
    }
}

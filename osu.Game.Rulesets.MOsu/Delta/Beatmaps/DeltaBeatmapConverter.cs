// Converter producing DeltaBeatmap so gimmick data has a home on the playable beatmap,
// and DeltaSlider so the removed 10x slider velocity cap (delta parity) applies to
// converted sliders.
...
{
    public class DeltaBeatmapConverter : OsuBeatmapConverter
    {
        public DeltaBeatmapConverter(IBeatmap beatmap, Ruleset ruleset)
            : base(beatmap, ruleset)
        {
        }

        protected override Beatmap<OsuHitObject> CreateBeatmap() => new DeltaBeatmap();
        ...
        {
            var beatmap = (DeltaBeatmap)base.ConvertBeatmap(original, cancellationToken);
            ...
            if (original is DeltaBeatmap mosuSource && mosuSource.Gimmicks.Parsed)
            {
                beatmap.Gimmicks = new DeltaGimmickData
                {
                    Sections = DeltaSectionGimmickEditorModel.CloneGimmicks(mosuSource.Gimmicks.Sections),
                    HitObjectGimmicks = new BeatmapHitObjectGimmicks
                    {
                        Entries = mosuSource.Gimmicks.HitObjectGimmicks.Entries.Select(DeltaClipboardContent.CloneEntry).ToList(),
                    },

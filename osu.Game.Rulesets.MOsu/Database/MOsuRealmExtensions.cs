...
{
    public static class MOsuRealmExtensions
    {
        ...
        public static Live<T> ToLive<T>(this T realmObject, MOsuRealmAccess realm)
            where T : RealmObject, IHasGuidPrimaryKey
        {
            return new MOsuRealmLive<T>(realmObject, realm);
        }
...
#pragma warning restore RS0030
        ...
        {
            if (!MOsuRealmAccess.CurrentThreadSubscriptionsAllowed)
                throw new InvalidOperationException($"Make sure to call {nameof(MOsuRealmAccess)}.{nameof(MOsuRealmAccess.RegisterForNotifications)}");

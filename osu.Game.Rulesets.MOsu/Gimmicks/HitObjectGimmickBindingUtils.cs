...
// adapted to host gimmick state on MosuHitObject instead of the core HitObject
...
namespace osu.Game.Rulesets.MOsu.Gimmicks
{
    public static class HitObjectGimmickBindingUtils
    {
        ...
        public static void EnsureObjectIds(IEnumerable<MosuHitObject> hitObjects)
        {
        ...
        public static void SynchroniseEntriesWithHitObjects(IEnumerable<MosuHitObject> hitObjects, BeatmapHitObjectGimmicks gimmicks)
        {
            ...
            var byObjectId = hitObjectList
                             ...
                             .ToDictionary(g => g.Key, g => new Queue<MosuHitObject>(g));
            ...
            var byStartTime = hitObjectList
                             ...
                             .ToDictionary(g => g.Key, g => new Queue<MosuHitObject>(g.OrderBy(v => v.ComboIndexWithOffsets).Select(v => v)));

            var entries = gimmicks.Entries;
            var claimedHitObjects = new HashSet<MosuHitObject>();
            var boundEntries = new List<(HitObjectGimmickEntry entry, MosuHitObject hitObject)>();
            ...
            for (int i = entries.Count - 1; i >= 0; i--)
            {
                ...
                MosuHitObject? matched = null;

                if (entry.ObjectId.HasValue
                    && byObjectId.TryGetValue(entry.ObjectId.Value, out Queue<MosuHitObject>? objectIdCandidates)
                ...
                if (matched == null
                    && byLegacyKey.TryGetValue((entry.StartTime, entry.ComboIndexWithOffsets), out Queue<MosuHitObject>? legacyCandidates)
                ...
                if (matched == null
                    && byStartTime.TryGetValue(entry.StartTime, out Queue<MosuHitObject>? timeCandidates)
        ...
        private static bool tryDequeueUnclaimed(Queue<MosuHitObject>? candidates, HashSet<MosuHitObject> claimed, out MosuHitObject? hitObject)
        {
        ...
        public static bool TryGetSettings(MosuHitObject hitObject,

    ...
    public static class MosuGimmickRuntime
    {
        ...
        public static void EnsureApplied(IBeatmap playableBeatmap, WorkingBeatmap? workingBeatmap, bool mutateList = true)
        {
            ...
            if (!data.Parsed)
            {
                ...
                (data.Sections, data.HitObjectGimmicks) = MosuGimmickParser.Parse(new StringReader(text));
                sliderVelocities = MosuGimmickParser.ParseSliderVelocity(new StringReader(text));
        ...
        /// </summary>
        private static void applyUncappedSliderVelocity(Beatmaps.MosuBeatmap mosuBeatmap, Dictionary<double, double>? sliderVelocities)
        {
            bool any = false;
            ...
            foreach (var slider in mosuBeatmap.HitObjects.OfType<Slider>())
            {
                ...
                // (MosuSlider.ApplyDefaultsToSelf) instead of the stock 10x-capped one.

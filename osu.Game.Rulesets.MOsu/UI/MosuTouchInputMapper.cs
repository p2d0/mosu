...
{
    /// <summary>
    /// Copy of upstream <see cref="osu.Game.Rulesets.Osu.UI.OsuTouchInputMapper"/> adapted for
    /// <see cref="MosuInputManager"/>. Upstream's mapper casts <c>KeyBindingContainer</c> to its
    /// private <c>OsuKeyBindingContainer</c>; mosu uses <c>MosuKeyBindingContainer</c>, so the cast
    /// throws <see cref="System.InvalidCastException"/> on touch. Mosu's own <c>AllowGameplayInputs</c>
    /// casts to <c>MosuKeyBindingContainer</c> and is safe.
    /// </summary>
    public partial class MosuTouchInputMapper : Drawable
    {
        /// <summary>
        ...
        private readonly MosuInputManager mosuInputManager;

        private Bindable<bool> tapsDisabled = null!;

        public MosuTouchInputMapper(MosuInputManager inputManager)
        {
            mosuInputManager = inputManager;
        }
        ...
        protected override bool OnTouchDown(TouchDownEvent e)
        {
            ...
            bool shouldResultInAction = mosuInputManager.AllowGameplayInputs && !tapsDisabled.Value && trackedTouches.All(t => t.Action != action);
            ...
            bool isDirectCircleTouch = mosuInputManager.CheckScreenSpaceActionPressJudgeable(e.ScreenSpaceTouchDownPosition);
            ...
            if (shouldResultInAction)
                mosuInputManager.KeyBindingContainer.TriggerPressed(action);
        ...
        private void updatePositionTracking(TrackedTouch newTouch)
        {
            ...
            {
                mosuInputManager.KeyBindingContainer.TriggerReleased(touchAction);
        ...
        private void handleTouchMovement(TouchEvent touchEvent)
        {
            ...
            if (!mosuInputManager.AllowUserCursorMovement)
                return;

            new MousePositionAbsoluteInput { Position = touchEvent.ScreenSpaceTouch.Position }.Apply(mosuInputManager.CurrentState, mosuInputManager);
        }
        ...
        protected override void OnTouchUp(TouchUpEvent e)
        {
            ...
            if (tracked.Action is OsuAction action)
                mosuInputManager.KeyBindingContainer.TriggerReleased(action);

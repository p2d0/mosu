...
namespace osu.Game.Rulesets.MOsu.Edit
{
    public partial class SectionGimmickToolboxGroup : EditorToolboxGroup
    {
        ...
        [Resolved]
        private MosuSectionGimmickEditorModel model { get; set; } = null!;
        ...
        private void trySelectSectionForHitObject(HitObject hitObject)
        {
            ...
            var section = (editorBeatmap.PlayableBeatmap as Beatmaps.MosuBeatmap)?.Gimmicks.Sections.FindSectionAt(hitObject.StartTime);
            if (section == null)
                return;
        ...
        private void bindSliderSetting(MosuFormSliderBar<float> slider, Action<SectionGimmickSettings, float> mutation, Func<float, float> clamp)
            => slider.Current.BindValueChanged(v => updateClampedSliderSetting(slider, mutation, v.NewValue, clamp));

        private void bindSliderSetting(MosuFormSliderBar<double> slider, Action<SectionGimmickSettings, double> mutation, Func<double, double> clamp)
            => slider.Current.BindValueChanged(v => updateClampedSliderSetting(slider, mutation, v.NewValue, clamp));
        ...
        private void updateClampedSliderSetting(MosuFormSliderBar<float> slider, Action<SectionGimmickSettings, float> mutation, float value, Func<float, float> clamp)
        {
            if (updatingControls)
                return;
        ...
        private void updateClampedSliderSetting(MosuFormSliderBar<double> slider, Action<SectionGimmickSettings, double> mutation, double value, Func<double, double> clamp)
        {
            if (updatingControls)
                return;

...
namespace osu.Game.Rulesets.MOsu.Tests.Delta.Gimmicks
{
    public partial class TestSceneFrumsEditorGimmicks : EditorTestScene
    {
        ...
        private DeltaGimmickData? preParsedGimmicks;
        ...
        public override void SetUpSteps()
        {
            ...
            AddStep("pre-parse gimmick sections", () =>
            {
                ...
                var (sections, hitObjectGimmicks) = DeltaGimmickParser.Parse(reader);

                preParsedGimmicks = new DeltaGimmickData
                {
                    Sections = sections,
        ...
        protected override IBeatmap CreateBeatmap(RulesetInfo ruleset)
        {
            ...
            // same step GetPlayableBeatmap performs), then wrap in a DeltaBeatmap carrying the
            // pre-parsed gimmicks (the editor test's working beatmap has no file access, so the
            // runtime cannot parse it itself).
            // Convert with the mosu converter so the playable carries DeltaSlider instances
            // (the real GetPlayableBeatmap path) and the uncapped-velocity assertions hold.
            var converted = new DeltaBeatmapConverter(source, new MosuRuleset()).Convert() as Beatmap<OsuHitObject>;
            ...
            var mosu = new DeltaBeatmap
            {
                ...
                Gimmicks = preParsedGimmicks ?? new DeltaGimmickData(),
            };
        ...
        public void TestGimmicksAppliedInEditor()
        {
            ...
            AddAssert("composer uses mosu editor ruleset", () => Editor.ChildrenOfType<DeltaEditorDrawableRuleset>().Any());

            AddUntilStep("gimmicks applied to playable beatmap", () =>
            {
                if (EditorBeatmap.PlayableBeatmap is not DeltaBeatmap mosuBeatmap)
                {
            ...
            AddAssert("fake notes resolvable in editor beatmap", () =>
            {
                ...
                var fake = mosuBeatmap.HitObjects.OfType<OsuHitObject>()
                                       .Select(o => DeltaGimmickApplier.CreateFakeObject(mosuBeatmap, mosuBeatmap.Gimmicks, o))
            ...
            AddAssert("editor drawable creation returns fake and hidden drawables", () =>
            {
                ...
                {
                    var d = DeltaGimmickRuntime.CreateGimmickDrawableRepresentation(mosuBeatmap, o);
            ...
            {
                if (EditorBeatmap.PlayableBeatmap is not DeltaBeatmap sourceMosu)
                    return false;
                ...
                var session = new DeltaGimmickData();
                var sessionEntry = new HitObjectGimmickEntry { ObjectId = DeltaGimmickApplier.GetObjectId(a), StartTime = a.StartTime, ComboIndexWithOffsets = a.ComboIndexWithOffsets, HitObjectIndex = aIndex + 1, Settings = new HitObjectGimmickSettings { IsFakeNote = true } };
                session.HitObjectGimmicks.Entries.Add(sessionEntry);

                // serialize, then reload into a fresh beatmap with fresh object instances
                string text = DeltaGimmickSerializer.Serialize(session.Sections, session.HitObjectGimmicks);
                using var reader = new StringReader(text);
                var reparsed = DeltaGimmickParser.Parse(reader);

                var freshBeatmap = new DeltaBeatmap { BeatmapInfo = sourceMosu.BeatmapInfo };
                ...
                DeltaGimmickApplier.Apply(freshBeatmap, freshBeatmap.Gimmicks);

                bool aFake = DeltaGimmickApplier.GetObjectSettings(freshBeatmap, freshBeatmap.Gimmicks, freshA)?.IsFakeNote == true;
                bool bFake = DeltaGimmickApplier.GetObjectSettings(freshBeatmap, freshBeatmap.Gimmicks, freshB)?.IsFakeNote == true;

                Logger.Log($"[TEST] reload: aFake={aFake} bFake={bFake} aIndex={aIndex} entryIndex={freshBeatmap.Gimmicks.HitObjectGimmicks.Entries.FirstOrDefault()?.HitObjectIndex} aId={DeltaGimmickApplier.GetObjectId(freshA)} bId={DeltaGimmickApplier.GetObjectId(freshB)}");
            ...
            AddAssert("fake note apply/clear data path", () =>
            {
                ...
                var fakeObj = DeltaGimmickApplier.CreateFakeObject(mosuBeatmap, mosuBeatmap.Gimmicks, target);
                ...
                bool cloneResolves = fakeObj != null
                    && DeltaGimmickApplier.GetObjectSettings(mosuBeatmap, mosuBeatmap.Gimmicks, fakeObj)?.IsFakeNote == true;

                var newEntry = mosuBeatmap.Gimmicks.HitObjectGimmicks.Entries.FirstOrDefault(e => e.StartTime == 14680);
                var resolved = DeltaGimmickApplier.GetObjectSettings(mosuBeatmap, mosuBeatmap.Gimmicks, target);
                ...
                bool cleared = DeltaGimmickApplier.CreateFakeObject(mosuBeatmap, mosuBeatmap.Gimmicks, target) == null;
            ...
            AddAssert("toolbox fake note add/delete + save persistence", () =>
            {
                ...
                // add fake
                model.SetSelectionBoolSetting((s, v) => s.IsFakeNote = v, true);
                bool applied = DeltaGimmickApplier.CreateFakeObject(mosuBeatmap, mosuBeatmap.Gimmicks, target) is FakeHitCircle or FakeSlider;
                ...
                bool saved = DeltaEditorSaver.Save(EditorBeatmap, realm, storage);
                ...
                // delete fake
                model.SetSelectionBoolSetting((s, v) => s.IsFakeNote = v, false);
                bool deleted = DeltaGimmickApplier.CreateFakeObject(mosuBeatmap, mosuBeatmap.Gimmicks, target) == null;
            ...
            AddAssert("toggle ON from a cleared entry sticks", () =>
            {
                ...
                Logger.Log($"[TEST] cleared-entry toggle ON: entryFake={after?.Settings.IsFakeNote} resolvedFake={resolved?.IsFakeNote} entryObjectId={after?.ObjectId} objId={DeltaGimmickApplier.GetObjectId(target)}");
            ...
            AddAssert("save writes .osu with gimmick sections", () =>
            {
                ...
                if (DeltaGimmickApplier.GetObjectSettings(mosuBeatmap, mosuBeatmap.Gimmicks, target)?.IsFakeNote != true)
                    model.SetSelectionBoolSetting((s, v) => s.IsFakeNote = v, true);

                if (!DeltaEditorSaver.Save(EditorBeatmap, realm, storage))
                    return false;
                ...
                var reparsed = DeltaGimmickParser.Parse(parseReader);

                var freshBeatmap = new DeltaBeatmap { BeatmapInfo = mosuBeatmap.BeatmapInfo };
                ...
                bool reloadedFake = freshTarget != null
                    && DeltaGimmickApplier.GetObjectSettings(freshBeatmap, freshBeatmap.Gimmicks, freshTarget)?.IsFakeNote == true;
            ...
            AddAssert("SV parser extracts raw timing point velocities", () =>
            {
                ...
                var sv = DeltaGimmickParser.ParseSliderVelocity(new StringReader(sample));
            ...
            AddAssert("mosu sliders use uncapped velocity formula", () =>
            {
                ...
                bool isDeltaSlider = slider is DeltaSlider;
                ...
                Logger.Log($"[TEST] slider sv={slider.SliderVelocityMultiplier} velocity={slider.Velocity} expected={expectedVelocity} isMosu={isDeltaSlider} uncapped={uncapped}");
                return isDeltaSlider && uncapped && Math.Abs(slider.Velocity - expectedVelocity) < 0.001;
            });

            AddUntilStep("slider velocity control uncapped", () =>
            {
                var composer = Editor.ChildrenOfType<DeltaHitObjectComposer>().Single();
            ...
            AddAssert("saved file preserves true slider velocity", () =>
            {
                ...
                var savedSv = DeltaGimmickParser.ParseSliderVelocity(new StringReader(text));
            ...
            AddAssert("no unsaved changes after load", () =>
                !Editor.ChildrenOfType<DeltaHitObjectComposer>().Single().HasUnsavedChanges);
        }
    }
}

...
namespace osu.Game.Rulesets.MOsu.Tests.Screens
{
    ...
    public partial class TestSceneResultsScreen : OsuManualInputManagerTestScene
    {
                ...
                dummyAPI.HandleRequest = request =>
                {
                    switch (request)
                    {
                        case ListTagsRequest listTagsRequest:
                        {
                            ...
                            Scheduler.AddDelayed(() => triggerSuccess(listTagsRequest, tags), 500);
                            return true;
                        }
                        ...
                        case GetBeatmapSetRequest getBeatmapSetRequest:
                        {
                            ...
                            Scheduler.AddDelayed(() => triggerSuccess(getBeatmapSetRequest, beatmapSet), 100);
                            return true;
                        }
                        ...
                        {
                            Scheduler.AddDelayed(() => triggerSuccess(request), 100);
                            return true;
                        }

                        case SearchBeatmapSetsRequest searchRequest:
                        {
                            ...
                            Scheduler.AddDelayed(() => triggerSuccess(searchRequest, new SearchBeatmapSetsResponse { BeatmapSets = sets }), 100);
                            return true;
                        }
                    }
        ...
        public void TestFetchScoresAfterShowingStatistics()
        {
            ...
            loadResultsScreen(() => screen = new DelayedFetchResultsScreen(createMosuScore(), tcs.Task));
        ...
        private TestResultsScreen createResultsScreen(ScoreInfo score = null) => new TestResultsScreen(score ?? createMosuScore());

        private static ScoreInfo createMosuScore() =>
            TestResources.CreateTestScoreInfo(TestResources.CreateTestBeatmapSetInfo(1, new[] { new MosuRuleset().RulesetInfo }).Beatmaps.First());

        private UnrankedSoloResultsScreen createUnrankedSoloResultsScreen() => new UnrankedSoloResultsScreen(createMosuScore());

        private static void triggerSuccess<T>(APIRequest<T> request, T response) where T : class =>
            typeof(APIRequest<T>).GetMethods(BindingFlags.NonPublic | BindingFlags.Instance)
                                 .FirstOrDefault(m => m.Name == "TriggerSuccess" && m.GetParameters().Length == 1 && m.GetParameters()[0].ParameterType == typeof(T))
                                 ?.Invoke(request, new object[] { response });

        private static void triggerSuccess(APIRequest request) =>
            typeof(APIRequest).GetMethod("TriggerSuccess", BindingFlags.NonPublic | BindingFlags.Instance, null, Type.EmptyTypes, null)
                              ?.Invoke(request, null);
        ...
        private class RulesetWithNoPerformanceCalculator : MosuRuleset
        {
